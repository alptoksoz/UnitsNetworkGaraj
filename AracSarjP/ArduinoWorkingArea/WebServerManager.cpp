...
void WebServerManager::begin() {
    ...
    server.on("/pair", HTTP_POST, [this]() { handlePairing(); });
...
void WebServerManager::handleStatus() {
    ...
    jsonBuffer["total_energy"] = status.totalEnergyWh;
...
void WebServerManager::handlePairing() {
    // Basit bir eşleşme yanıtı
    jsonBuffer.clear();
    jsonBuffer["status"] = "success";
    jsonBuffer["message"] = "Paired successfully";
    
    serializeJson(jsonBuffer, buffer);
    server.send(200, "application/json", buffer);
    
    // Eşleşme durumunu SystemManager'a bildir
    systemManager->handlePairing();
}
